    ...
    try {
      ...
      if (mounted) {
        if (widget.isCommissioning) {
          Navigator.of(context).pop(); // Go back to Installer Dashboard
          ScaffoldMessenger.of(context).showSnackBar(
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Configuration failed: $e'), backgroundColor: Colors.red),
        );
      }
    }
  }
            ...
            SliverToBoxAdapter(
              child: Padding(
                ...
                child: ElevatedButton.icon(
                  onPressed: (remaining < 0) ? null : _saveAndApply,
                  icon: const Icon(Icons.save_alt),
                  label: const Text("APPLY TO CONTROLLER"),
